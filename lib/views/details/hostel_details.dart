...
//   const HostelDetails({super.key, required this.hostel, this.qrUrl});
...
//     if (widget.hostel.sharings.isNotEmpty) {
//       return widget.hostel.sharings.any(
//         (s) => s.type!.trim().toUpperCase() == 'AC',
//       );
//     }

//     // Fallback to type field
//     final types = widget.hostel.type
...
class _HostelDetailsState extends State<HostelDetails>
  ...
  // Separate AC and Non-AC sharings
  List<SharingOption> get _acSharings {
    return widget.hostel.sharings
        .where((s) => s.type?.toUpperCase() == 'AC')
        .toList();
  }

  List<SharingOption> get _nonAcSharings {
    return widget.hostel.sharings
        .where((s) => s.type?.toUpperCase() == 'NON-AC')
        .toList();
  }

  bool get _hasAcPrices => _acSharings.isNotEmpty;
  bool get _hasNonAcPrices => _nonAcSharings.isNotEmpty;
  bool get _hasFeatures => widget.hostel.features.isNotEmpty;
  bool get _hasFurnishing => widget.hostel.furnishing.isNotEmpty;
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // Show AC and Non-AC prices separately
                      if (_hasNonAcPrices) ...[
                        _PriceSectionHeader(
                          title: 'Non-AC Rooms',
                          color: const Color(0xFFE53935),
                      ...
                      if (!_hasAcPrices && !_hasNonAcPrices)
                        const Text(
                          'No pricing info available.',
                          style: TextStyle(color: Colors.black45, fontSize: 13),
                        ),
...
class _PriceSectionHeader extends StatelessWidget {
  ...
  const _PriceSectionHeader({required this.title, required this.color});
...
class _TypeBadge extends StatelessWidget {
  ...
  const _TypeBadge({required this.hasAc, required this.hasNonAc});
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        if (hasAc && hasNonAc) const SizedBox(width: 6),
        if (hasAc)
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
            decoration: BoxDecoration(

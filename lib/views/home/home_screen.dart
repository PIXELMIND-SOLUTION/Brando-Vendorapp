...
class _HifiDetailsScreenState extends State<HifiDetailsScreen>
  ...
  String _findPrice(
    ...
    try {
      ...
      price ??= isMonthly ? match.monthlyPrice : match.dailyPrice;
      if (price == null || price.isNaN || price.isInfinite) return '0';
  ...
  /// Safely parse a controller value — always returns a real number, never NaN.
  double _parsePrice(TextEditingController? controller) {
    if (controller == null) return 0;
    final text = controller.text.trim();
    if (text.isEmpty) return 0;
    final parsed = double.tryParse(text);
    // double.tryParse returns null on failure, which we map to 0
  ...
  void _saveAndGoBack() {
    final List<SharingOption> sharings = _shareKeys.map((key) {
      ...
      if (_isAcEnabled) {
        // AC toggle ON → send both AC and Non-AC prices; use 0 (not null)
        return SharingOption(
          shareType: key,
          acMonthlyPrice: acMonthly,
          acDailyPrice: acDaily,
          nonAcMonthlyPrice: nonAcMonthly,
          nonAcDailyPrice: nonAcDaily,
          // Always provide monthlyPrice / dailyPrice as well so the
          // backend never receives a null/NaN for these paths.
          monthlyPrice: acMonthly,
          dailyPrice: acDaily,
        );
      } else {
        // Non-AC only → every numeric field must be a real number
        return SharingOption(
          shareType: key,
          type: 'Non-AC',
          monthlyPrice: nonAcMonthly,
          dailyPrice: nonAcDaily,
          // Explicitly zero-out AC fields so the model never sends NaN
          acMonthlyPrice: 0,
          acDailyPrice: 0,
          nonAcMonthlyPrice: nonAcMonthly,
          nonAcDailyPrice: nonAcDaily,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      // ── FIX 1: let Flutter resize the layout when keyboard appears ──
      ...
      // ── FIX 2: entire body is one scrollable column ─────────────────
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // ── FIX 3: TabBarView replaced with AnimatedBuilder ───────
            // TabBarView inside a SingleChildScrollView needs a fixed
            // height. We instead listen to the tab index and show the
            // correct price section directly — no Expanded needed.
            AnimatedBuilder(
  ...
  Widget _buildGrid(Map<String, TextEditingController> prices, Color color) {
    final keys = prices.keys.toList();
    return GridView.builder(
      // ── FIX 4: grid must never scroll inside the outer scroll ───────
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
        crossAxisCount: 3,

...
class _SplashScreenState extends State<SplashScreen>
  ...
  void initState() {
    ...
    _topRightOpacity = Tween<double>(
      begin: 0,
      end: 1,
    ).animate(interval(0.0, 0.4));
    _topRightScale = Tween<double>(
      begin: 0.5,
      end: 1.0,
    ...
    _centerOpacity = Tween<double>(
      begin: 0,
      end: 1,
    ).animate(interval(0.15, 0.55));
    _centerScale = Tween<double>(
      begin: 0.4,
      end: 1.0,
    ...
    _bottomLeftOpacity = Tween<double>(
      begin: 0,
      end: 1,
    ).animate(interval(0.3, 0.65));
    _bottomLeftScale = Tween<double>(
      begin: 0.5,
      end: 1.0,
...
//     );
//     _otpOpacity = Tween<double>(
//       begin: 0,
//       end: 1,
//     ).animate(CurvedAnimation(parent: _otpAnimCtrl, curve: Curves.easeOut));
...
//       _otpAnimCtrl.forward();
//       _startResendTimer();
//     } else if (provider.isVendorExists == true &&
//         provider.adminApproved == false) {
//       // Vendor exists but not approved by admin
//       _showError(provider.errorMessage);
...
//         if (mounted) {
//           Navigator.of(context).pop();
//           provider.resetLoginApprovalStatus();
//         }
//       });
//     } else if (provider.isVendorExists == false && !widget.autoSendOtp) {
//       // Vendor doesn't exist - navigate to registration
//       Navigator.of(context).pop();
//       showModalBottomSheet(
//         context: context,
//         isScrollControlled: true,
...
class _GetOTPModalState extends State<GetOTPModal>
  ...
  //   } else if (provider.isVendorExists == true &&
  //       provider.adminApproved == false) {
  //     // Toast already shown in provider, just close modal after delay
  //     Future.delayed(const Duration(seconds: 2), () {
  //       if (mounted) {
  //         Navigator.of(context).pop();
  //         provider.resetLoginApprovalStatus();
  ...
  //     );
  //   }
  //   // Error toast already shown in provider, no need for else block
  // }
  // Future<void> _sendOTP() async {
  //   if (_mobileController.text.length < 10) {
  //     ToastService.showError('Please enter a valid 10-digit mobile number');
  ...
  //   } else if (provider.isVendorExists == true &&
  //       provider.adminApproved == false) {
  //     // Get vendorId from SharedPreferences
  //     final vendorId = await SharedPreferenceHelper.getVendorId();
  //     print('Retrieved vendorId for approval modal: $vendorId'); // Debug log
  ...
  //         isDismissible: false,
  //         enableDrag: false,
  //         builder: (_) => ApprovalStatusModal(
  //           approvalStatus: provider.loginApprovalStatus ?? 'pending',
  //           vendorId: vendorId,
  //         ),
  //       );
  //     } else if (mounted && vendorId == null) {
  //       // Fallback - just show error if no vendorId
  //       ToastService.showError('Unable to check approval status');
  //     }
  ...
  Future<void> _sendOTP() async {
    ...
    if (success) {
      ...
      _startResendTimer();
        ...
        provider.adminApproved == false) {
      // Get vendorId from SharedPreferences BEFORE closing the modal
      final vendorId = await SharedPreferenceHelper.getVendorId();
      print('Retrieved vendorId before closing modal: $vendorId');
      ...
      // Use a short delay to ensure the modal is fully closed
      await Future.delayed(const Duration(milliseconds: 100));

      // Show approval modal only if we have vendorId and context is still valid
      if (mounted && vendorId != null && vendorId.isNotEmpty) {
        ...
        // Show error if no vendorId found
  ...
  Future<void> _verifyOTP() async {
    if (_otpController.text.isEmpty) {
      ToastService.showError('Please enter OTP');
      return;
    }
  ...
  Future<void> _verifyOtp() async {
    ...
    if (success) {
      ...
      if (approvalStatus == 'approved') {
        ...
        final String? vendorId = await SharedPreferenceHelper.getVendorId();
...
//   final String? vendorId; // Add this parameter

//   const ApprovalStatusModal({
//     super.key,
//     required this.approvalStatus,
//     this.vendorId, // Add this
//   });
...
//   void _startPolling() {
//     print('Starting polling for vendorId: ${widget.vendorId}'); // Debug log
...
//     if (_isPolling || !mounted || widget.vendorId == null) {
//       print(
//         'Cannot check status - isPolling: $_isPolling, mounted: $mounted, vendorId: ${widget.vendorId}',
//       );
//       return;
//     }
//     _isPolling = true;
...
//     try {
//       print('Checking approval status for vendorId: ${widget.vendorId}');

//       final uri = Uri.parse(
//         'http://187.127.146.52:2003/api/vendors/${widget.vendorId}/approval-status',
//       );
//       final response = await http.get(uri);

//       print('Approval status response status: ${response.statusCode}');
//       print('Approval status response body: ${response.body}');
...
//         if (approvalStatus == 'approved') {
//           print('Vendor approved! Navigating to home...');
...
//         }
//       } else {
//         print('Failed to check approval status: ${response.statusCode}');
//       }
//     } catch (e) {
...
//   @override
//   Widget build(BuildContext context) {
//     final Color accentColor = _isPending
//         ? const Color(0xFFFF8F00)
//         : const Color(0xFFE53935);
//     final Color bgColor = _isPending
//         ? const Color(0xFFFFF8E1)
//         : const Color(0xFFFFEBEE);
//     final IconData icon = _isPending
//         ? Icons.hourglass_top_rounded
//         : Icons.cancel_outlined;
//     final String title = _isPending ? 'Awaiting Approval' : 'Request Rejected';
...
//     return AnimatedBuilder(
//       animation: _animCtrl,
//       builder: (_, child) => Opacity(opacity: _opacityAnim.value, child: child),
//       child: Container(
//         decoration: const BoxDecoration(
//           color: Colors.white,
//           borderRadius: BorderRadius.vertical(top: Radius.circular(28)),
...
class ApprovalStatusModal extends StatefulWidget {
  final String approvalStatus;
  final String vendorId; // Make it required, not nullable

  const ApprovalStatusModal({
    super.key,
    required this.approvalStatus,
    required this.vendorId, // Required now
  });
...
class _ApprovalStatusModalState extends State<ApprovalStatusModal>
  ...
  void initState() {
    ...
    if (_isPending && widget.vendorId.isNotEmpty) {
      print('Starting polling for vendorId: ${widget.vendorId}');
      _startPolling();
    }
  }
  ...
  Future<void> _checkApprovalStatus() async {
    if (_isPolling || !mounted) {
      print('Skip check - isPolling: $_isPolling, mounted: $mounted');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (approvalStatus == 'approved') {
          ...
          if (mounted) {
            // Get the token and complete login
            // You might need to call a method to complete the login process

            Navigator.of(context).pop(); // Close approval modal
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (_) => const NavbarScreen()),
            );

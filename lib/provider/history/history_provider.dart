...
class HistoryProvider extends ChangeNotifier {
  List<RoomBookingData> _bookings = [];
  List<Booking> _joiners = []; // Add joiners list
  bool _isLoading = false;
  String _errorMessage = '';
  bool _hasError = false;

  List<RoomBookingData> get bookings => _bookings;
  List<Booking> get joiners => _joiners; // Add getter for joiners
  bool get isLoading => _isLoading;
  String get errorMessage => _errorMessage;
  bool get hasError => _hasError;
  ...
  Future<void> fetchHistory(String vendorId) async {
    ...
    _hasError = false;
    _errorMessage = '';
    _joiners = []; // Clear joiners before fetching
    notifyListeners();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse['data'] != null) {
          _bookings = (jsonResponse['data'] as List)
        ...
        // Parse joiners data from the response
        if (jsonResponse['joiners'] != null) {
          _joiners = (jsonResponse['joiners'] as List)
        ...
        _errorMessage = '';
        print(
          'Loaded ${_bookings.length} rooms and ${_joiners.length} joiners',
        );
      } else {
        _hasError = true;
        _errorMessage =
